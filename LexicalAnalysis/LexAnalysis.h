...
struct token
{
    ...
    string keyWord;
    token() {}
    token(string word, int id1, int t)
    {
        keyWord = word;
        id = id1;
        type = t;
...
int LexicalAnalysis::get_operator(int pos, string word)
{
    ...
    if (i < prog.size() - 1 && !isspace(prog[i + 1]))
    {
        ...
        case '%':
            ...
            else if (isLetter(peek))
            {
                word += peek;
                // 标识符
                res.push_back(token(word, 81, ID));
        ...
        case '/':
            if (peek == '/')
            {
                ...
                // 注释
                res.push_back(token(word, 79, OPERATOR));
                return pos + word.length() - 1;
            }
            else if (peek == '*')
            {
    ...
    res.push_back(token(word, tokenMap[word], OPERATOR));
    return pos + word.length() - 1;
}
...
int LexicalAnalysis::get_number(int pos, string word)
{
    ...
    res.push_back(token(word, 80, NUM));
    return pos + word.length() - 1;
}
...
int LexicalAnalysis::get_letter(int pos, string word)
{
    ...
    {
        res.push_back(token(word, tokenMap[word], RESERVED));
    }
    else
    {
        res.push_back(token(word, 81, ID));
...
void LexicalAnalysis::token_scan(string raw_prog)
{
    ...
    for (int i = 0; i < prog.length(); i++)
    {
        ...
        if (!isspace(prog[i]))
        {
            ...
            {
                res.push_back(token(word, tokenMap[word], DELIMITER));
            }

            // 错误处理
            if (flag > 1) // 如果两个token之间没有空格隔开
            {
                // 不合法的标识符
                if (res[res.size() - 2].type == NUM && res[res.size() - 1].type == ID)
                {
                    cout << "Error with the " << res.size() - 2 << " token, word\"" << res[res.size() - 2].keyWord + res[res.size() - 1].keyWord << "\" is wrong. ";
                    cout << "Illegal identifiers: Numbers cannot be written before letters" << endl;
                }
                // 不合法的运算符
